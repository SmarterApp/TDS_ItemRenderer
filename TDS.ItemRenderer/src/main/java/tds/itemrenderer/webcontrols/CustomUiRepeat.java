 ...
 *  
 */
public class CustomUiRepeat extends UIComponentBase implements NamingContainer
{
  public static final String                COMPONENT_TYPE                   = "facelets.ui.Repeat";

  public static final String                COMPONENT_FAMILY                 = "facelets";

  // private static final String SKIP_ITERATION_HINT =
  ...
  // will be set to false if the data should not be refreshed at the beginning
  // of the encode phase
  ...
  private RepeatStatus _getRepeatStatus () {
    return new RepeatStatus (_count == 0, _index + getStep () >= getDataModel ().getRowCount (), _count, _index, getOffset (), _end, getStep ());
  }

  protected void _captureScopeValues () {
    String var = getVar ();
    if (var != null) {
  ...
  protected boolean _isIndexAvailable () {
    return getDataModel ().isRowAvailable ();
  }

  protected void _restoreScopeValues () {
    String var = getVar ();
    if (var != null) {
   ...
   * the view is serialized. Transient components are skipped (no state is saved
   * for them).
   * <p>
  ...
  private Collection<Object[]> saveDescendantComponentStates (UIComponent parent, boolean iterateFacets, boolean saveChildFacets) {
    ...
    // Index to indicate how many components has been passed without state to
    // save.
      ...
      while (childIterator.hasNext ()) {
        ...
        if (!child.isTransient ()) {
          ...
          if (child instanceof EditableValueHolder) {
            ...
            childStates.add (child.getChildCount () > 0 ? new Object[] { new SavedState ((EditableValueHolder) child), saveDescendantComponentStates (child, saveChildFacets, true) } : new Object[] {
                new SavedState ((EditableValueHolder) child), null });
  ...
  protected void _setIndex (int index) {
    // save child state
    // _saveChildState();
    if (index < -1) {
    ...
    if (_index == -1) {
      ...
      // If no initial component state, there are no EditableValueHolder
      // instances,
      // and that means there is no state to be saved for the current row, so we
      // can
  ...
  protected void _validateAttributes () throws FacesException {
    int begin = getOffset ();
  ...
  public boolean visitTree (VisitContext context, VisitCallback callback) {
    ...
    // (Boolean)
    // context.getFacesContext().getAttributes().get(SKIP_ITERATION_HINT);
  ...
  protected void customRenderer (FacesContext context, UIComponent component) throws IOException {
    if (component.getChildCount () > 0) {
      ...
      Object currentDataRow = getDataModel ().getRowData ();
      IUiRepeatDataElement reBindableData = null;
      if (currentDataRow instanceof IUiRepeatDataElement)
        reBindableData = (IUiRepeatDataElement) currentDataRow;

      for (int i = 0, childCount = component.getChildCount (); i < childCount; i++) {
        UIComponent child = component.getChildren ().get (i);
        if (reBindableData != null && child instanceof IRebindableComponent) {
          String repeaterLocalName = ((IRebindableComponent) child).getRepeaterElementName ();
          if (!StringUtils.isEmpty (repeaterLocalName)) {
            UIComponent replacementComponent = reBindableData.getCustomBinding (repeaterLocalName);
            if (replacementComponent != null)
              child = replacementComponent;
          }
        }
  ...
  private final class IndexedEvent extends FacesEvent
  {
    ...
    public IndexedEvent (CustomUiRepeat owner, FacesEvent target, int index) {
      super (owner);
      _target = target;
      _index = index;
    }
    ...
    public void processListener (FacesListener listener) {
      CustomUiRepeat owner = (CustomUiRepeat) getComponent ();
   ...
   * @SuppressWarnings("unchecked")
   * 
   * @Override public void restoreState(FacesContext facesContext, Object state)
   * { if (state == null) { return; }
   * 
   * Object[] values = (Object[])state;
   * super.restoreState(facesContext,values[0]); if (values[1] == null) {
   * _rowStates.clear(); } else { _rowStates = (Map<String,
   * Collection<Object[]>>) restoreAttachedState(facesContext, values[1]); } }
   * 
   * @Override public Object saveState(FacesContext facesContext) { if
   * (initialStateMarked()) { Object parentSaved =
   * super.saveState(facesContext); if (parentSaved == null &&
   * _rowStates.isEmpty()) { //No values return null; } return new
   * Object[]{parentSaved, saveAttachedState(facesContext, _rowStates)}; } else
   * { Object[] values = new Object[2]; values[0] =
   * super.saveState(facesContext); values[1] = saveAttachedState(facesContext,
   * _rowStates); return values; } }
   */
  ...
  enum PropertyKeys {
    value, var, size, varStatus, offset, step
  }
}
