...
public class DummyItsContentGeneratorLayoutMulti
{
  private static int ITSDocumentID = 0;

  public static ItemRenderGroup getDummyContent () {
    ItemRenderGroup group = new ItemRenderGroup ("page-3779685989", "default", "ENU");
    group.setPassage (createDummyItsPassage ());
    group.setPrinted (false);

    group.add (createItem1 ());
    group.add (createItem2 ());
    group.add (createItem3 ());
    return group;
  }

  private static IITSDocument createDummyItsPassage () {
    IITSDocument doc = new ITSDocument();
